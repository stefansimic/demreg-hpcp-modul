...
from dem_inv_gsvd_vectorized import dem_inv_gsvd_vectorized
from dem_reg_map_vectorized import dem_reg_map_vectorized
import concurrent.futures
from concurrent.futures import ProcessPoolExecutor, as_completed
from tqdm import tqdm
from threadpoolctl import threadpool_limits

def demmap_pos_vectorized(dd,ed,rmatrix,logt,dlogt,glc,reg_tweak=1.0,max_iter=10,rgt_fact=1.5,dem_norm0=None,nmu=42,warn=False,l_emd=False,rscl=False):
    dd=np.array(dd)
    ed=np.array(ed)
...
def dem_pix(dnin,ednin,rmatrix,logt,dlogt,glc,reg_tweak=1.0,max_iter=10,rgt_fact=1.5,dem_norm0=None,nmu=42,warn=True,l_emd=False,rscl=False):
    ...
    if dem_norm0 is None or (np.size(dem_norm0)==1 and dem_norm0==0):
        ...
        rgt=reg_tweak
        sva,svb,U,V,W=dem_inv_gsvd_vectorized(rmatrixin.T,L)
        lamb=dem_reg_map_vectorized(sva,svb,U,W,dn,edn,rgt,nmu)
