#!/usr/bin/env python3
"""
bench_demreg.py — Dask version

Purpose
-------
Baseline benchmark for the Dask-optimized DEMREG pipeline with guaranteed artifact outputs:
...
import numpy as np
# === DASK-enabled DEMREG entry point ===
...
def run_once(payload):
    ...
    dem, edem, elogt, chisq, dn_reg = dn2dem_pos(payload["dn"], payload["edn"], payload["tresp"], payload["tresp_logt"], payload["temps"])
...
def main():
    ap = argparse.ArgumentParser(description="Dask DEMREG benchmark with NPZ + preview outputs")
